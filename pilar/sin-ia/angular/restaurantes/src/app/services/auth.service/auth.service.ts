...
export class AuthService {
	...
	private clearSession(clearCookie = true): void {
		this.token.set(null);
		this.currentUserState.set(null);
	...
	private readTokenCookie(): string | null {
		const str = this.parseDecodedCookie('api_recetas_token=');
		return str || null;
	}
	...
	private writeTokenCookie(token: string): void {
		...
		document.cookie = `${this.tokenCookieName}=${token}; expires=${expires}; path=/`;
	}
	...
	private parseDecodedCookie(name: string): string | null {
		const nameEQ = name;
